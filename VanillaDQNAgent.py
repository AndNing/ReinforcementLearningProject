...
class VanillaDQNAgent:
    ...
    def __init__(
        ...
        self.dqn = VanillaDQN(obs_dim, action_dim).to(self.device)
        self.dqn_target = VanillaDQN(obs_dim, action_dim).to(self.device)
    ...
    def train(self, num_frames: int, plotting_interval: int = 2000):
        ...
        for frame_idx in range(1, num_frames + 1):
            ...
            if len(self.memory) >= self.batch_size:
                ...
                self.epsilon_threshold = self.min_epsilon + (self.max_epsilon - self.min_epsilon) * math.exp(
                    -1. * self.count / self.epsilon_decay)
    ...
    def _plot(
        ...
        plt.xlabel('number of completed episode')
        plt.plot(scores)
        plt.subplot(132)
        plt.title('loss')
